...
def login_view(request):
    if request.method == 'POST':
        ...
        if user.check_password(password):
            ...
            if username == 'nutri@gmail.com' and password == 'nutri':
...
def show_nutrition(request):
    nutrition = Nutrition.objects.all()
    return render(request, 'show_nutrition.html', {'nutrition': nutrition})

def show_meditation(request):
    meditation = Meditation.objects.all()
    return render(request, 'show_mediation.html', {'meditation': meditation})

def show_fitness(request):
    fitness = Fitness.objects.all()
    return render(request, 'show_fitness.html', {'fitness':fitness})

def nutrition_details(request):
    # Your view logic here
    return render(request, 'mainapp/login_view/nutrition/show_nutrition.html')
